...
def read_game_espn_id(game_key, existing_game_ids_dict={}, read_new_game_ids=True):
	...
	if game_key in existing_game_ids_dict.keys():
		...
		if len(game_data) > 2:
			...
			away_team = converter.convert_team_abbrev_to_name(away_team_abbrev)
			home_team = converter.convert_team_abbrev_to_name(home_team_abbrev)

			full_game_key = away_team + ' ' + home_team + ' ' + date
...
def read_player_season_logs(player_name, player_espn_id='', read_x_seasons=1, player_espn_ids={}, season_year=2024, all_game_logs={}, todays_date=datetime.today().strftime('%m-%d-%y'), current_year_str='', player_teams={}):
...
def read_all_players_season_logs(player_names, read_x_seasons=1, player_espn_ids={}, season_year=2024, all_players_teams={}):
	...
	print('input: player names = players of interest')
...
def read_all_players_teams(player_espn_ids_dict, read_new_teams=False):
	...
	print('input: player_espn_ids_dict = player:id,...')
	print('output: player:year:team:gp,...\n')
...
def read_all_players_in_games(all_player_season_logs_dict, all_players_teams, cur_yr, init_player_stat_dicts={}, read_new_game_ids=True, season_parts=['regular','postseason','full']):#, season_year=2024):
	...
	#all_players_in_games_dict = {} # {player:{game:{teammates:[],opponents:[]}}}
	all_players_in_games_dict = copy.deepcopy(init_box_scores) # change to all_box_scores
					...
					try:
						...
						for game_idx, row in season_part_game_log.iterrows():
							...
							if not game_key in all_players_in_games_dict[season_year].keys():
								...
								if re.search('too many requests', game_espn_id): 
	...
	if not init_box_scores == all_players_in_games_dict:
		writer.write_cur_and_prev(init_box_scores, all_players_in_games_dict, cur_box_scores_file, prev_box_scores_file, cur_yr)
...
def read_player_abbrev(player, all_players_teams, season_year, all_players_in_games_dict, cur_yr='', rosters={}):
	print('\n===Read Player Abbrev: ' + player.title() + '===\n')

	player_abbrev = ''

	year_players_in_games_dict = all_players_in_games_dict[season_year]
		...
		for loc, game_team_players in game_players.items():
			...
			for team_part in game_team_players.values():
				...
				for abbrev in team_part:
					...
					player_name = determiner.determine_player_full_name(abbrev, team, all_players_teams, game_key, cur_yr, rosters)
...
def read_all_players_abbrevs(all_players_in_games_dict, all_players_teams, cur_yr, todays_date, rosters={}):
				...
				for loc, game_team_players in game_players.items():
					...
					for team_part in game_team_players.values():
						...
						for player_abbrev in team_part:
							...
							if player_abbrev not in year_players_abbrevs.values() and player_abbrev not in unknown_names:

								player_name = determiner.determine_player_full_name(player_abbrev, team, all_players_teams, game_key, cur_yr, rosters)
...
def read_team_players(team, year_players_in_games_dict, all_players_teams, all_players_abbrevs={}, cur_yr='', rosters={}):
						...
						if len(all_players_abbrevs.keys()) > 0:
							for name, abbrev in all_players_abbrevs.items():
								if player == abbrev:
									player_name = name
						if player_name == '':
							player_name = determiner.determine_player_full_name(player, team, all_players_teams, game_key, cur_yr, rosters)#converter.convert_player_abbrev_to_name(player)
...
# year_players_in_games_dict = {game:{away:{starters:[],bench:[]},home:starters:[],bench:[]}}
def read_season_teams_players(year, year_players_in_games_dict, init_all_teams_players, teams, all_players_teams, all_players_abbrevs, cur_yr='', rosters={}):
	...
	for team in teams:
		if team not in season_teams_players.keys():
			team_players = read_team_players(team, year_players_in_games_dict, all_players_teams, all_players_abbrevs, cur_yr, rosters)
	...
	# players_in_game_dict = {away:{starters:[],bench:[]},home:starters:[],bench:[]}}
	# for game, players_in_game_dict in year_players_in_games_dict.items():
	# 	# get teams in game
	...
	# 		# add teams players to all dict
	# 		for team, team_players in init_all_teams_players.items():
	# 			if re.search(team, game):
	# 				if team not in season_teams_players.keys():
	# 					season_teams_players[team] = []

	# 				loc = 'home'
	# 				if team == away_team:
	# 					loc = 'away'

	# 				game_team_players = players_in_game_dict[loc]
	# 				for player in game_team_players:
	# 					if player not in season_teams_players[team].keys():
	# 						season_teams_players[team].append(player)
					
	# 	if game not in season_teams_players.keys():

	# 		season_teams_players[year] = season_teams_players
...
def read_all_teams_players(all_players_in_games_dict, rosters, cur_yr, todays_date, all_players_teams, all_players_abbrevs):
	...
	# so update teams after each game
	# always current yr bc no matter what yr of interest only current yr changes with each new game
	if cur_yr == '':
		cur_yr = determiner.determine_current_season_year() #str(datetime.today().year)
	all_cur_teams_players_filename = 'data/all teams players - ' + cur_yr + ' - ' + todays_date + '.json'
	all_prev_teams_players_filename = 'data/all teams players - prev.json'
	init_all_teams_players = read_cur_and_prev_json(all_cur_teams_players_filename,all_prev_teams_players_filename)
	print('init_all_teams_players: ' + str(init_all_teams_players))
	all_teams_players = copy.deepcopy(init_all_teams_players) # all teammates for all players

	# need to compare teammates from box score to teammates still on roster to make sure they are still on roster
	# before saying they are on bench

	# loop thru all players in games to assemble teams
	# year_players_in_games_dict = {game:{away:{starters:[],bench:[]},home:starters:[],bench:[]}}
	for year, year_players_in_games_dict in all_players_in_games_dict.items():
		# player_teammates = {year:[teammates],...}
		season_teams_players = read_season_teams_players(year, year_players_in_games_dict, all_teams_players, teams, all_players_teams, all_players_abbrevs, cur_yr, rosters)

		all_teams_players[year] = season_teams_players


	if not init_all_teams_players == all_teams_players:
		writer.write_cur_and_prev(init_all_teams_players, all_teams_players, all_cur_teams_players_filename, all_prev_teams_players_filename, cur_yr)
...
def read_all_players_teammates(all_player_season_logs_dict, all_players_in_games_dict, cur_yr, todays_date):
	...
	if not init_all_teammates == all_players_teammates:
		writer.write_cur_and_prev(init_all_teammates, all_players_teammates, all_cur_teammates_filename, all_prev_teammates_filename, cur_yr)
			...
			for team_idx in range(len(lineups_lists)):
				...
				if lineup_team in rosters.keys():
					...
					for player_element in lineup_list.find_all('li', {'class': 'lineup__player'}):
						...
						print('player_name: ' + str(player_name))
						player_name = determiner.determine_player_full_name(player_name, lineup_team, all_players_teams, cur_yr=cur_yr, rosters=rosters)
						#print('player_name: ' + str(player_name))
